...
class ResumeScanner():
    ...
    def __init__(self, resume_file, jd_file, domain, industry, affinda_token, temp_folder):
    ...
    def file_type(self):
        ...
        if self.extension == 'docx' or 'pdf' or 'html':
            # print(f"Your resume is '{extension}', which can be easily scanned by ATS")
            self.ATSp += 2.5

            self.resume_scores['best_practices'][
                'file_type'] = f"Your resume is '{self.extension}', which can be easily scanned by ATS"
        else:
            self.resume_scores['best_practices'][
                'file_type'] = f"Your resume is '{self.extension}', which can not be easily scanned by ATS"
    ...
    def best_practices(self):
        ...
        if Found != None:
            ...
            self.resume_scores['best_practices']['section_headings']['pop up'] = [
        ...
        if (orgName in jobDescTemp and orgURL in jobDescTemp):
            self.resume_scores['best_practices'][
                "Organisation's Name & Website"] = f"Add the organization Name '{orgName}' and URL {orgURL} are present in job description"
            self.ATSp += 2.5
        elif orgName in jobDescTemp:
            self.resume_scores['best_practices'][
                "Organisation's Name & Website"] = f"Add the organization Name '{orgName}' is present in job description"
            self.ATSp += 2.5
        elif orgURL in jobDescTemp:
            self.resume_scores['best_practices'][
                "Organisation's Name & Website"] = f"Add the organization URL '{orgURL}' is present in job description"
            self.ATSp += 2.5
        else:
            self.resume_scores['best_practices'][
                "Organisation's Name & Website"] = f"Add the organisation’s name (the potential employer or advertiser) and the website address (URL) for this job advert."
        ...
        if len(unkR) > 0:
            self.resume_scores['best_practices'][
                'Special Characters'] = f"Unknown/Unreadable Characters are present in your CV/Resume: " + ''.join(
                u for u in unkR)
        else:
            self.ATSp += 2.5
            self.resume_scores['best_practices'][
                'Special Characters'] = f"There are no unknown/unreadable characters on your resume."
